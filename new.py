...
ID = data.iloc[0]['ID']
Weight = data.iloc[0]['BW']
ID = ID.replace("Trovan unique", "").strip()  # 去除“Trovan unique”部分

# 目录路径
dir = './datanew/20240319/'
...
for filename in os.listdir(dir_path):
    if filename.startswith(ID) and filename.endswith('color_24.tiff'):
...
if image_path:
    ...
    print(f"没有找到以 {ID} 开头并且以 color_24.tiff 结尾的文件。")
...
# 输出结果
if contours:
    print(f"鱼的长度：{fish_length} 像素")
    print(f"鱼的高度：{fish_height} 像素")
    fish_area = cv2.contourArea(largest_contour)
    print(f"面积：{fish_area} 像素")
...
# # HSV转换与蓝色掩膜创建
# plt.subplot(3, 2, 2)
# plt.title('步骤2：蓝色掩膜（原始掩膜）')
# plt.imshow(mask, cmap='gray')  # 以灰度图显示掩膜效果
# plt.axis('off')
#
# # 反转掩膜以关注非蓝色区域（鱼）
# plt.subplot(3, 2, 3)
# plt.title('步骤3：反转掩膜')
# plt.imshow(result_inverted, cmap='gray')  # 以灰度图显示
# plt.axis('off')
#
# # 裁剪后的中心区域
# plt.subplot(3, 2, 4)
# plt.title('步骤4：裁剪后的中心区域')
# plt.imshow(cropped_result, cmap='gray')  # 以灰度图显示
# plt.axis('off')

# 最终结果
plt.subplot(3, 2, 5)
plt.title('步骤5：带有包围框的最终结果')
plt.imshow(cv2.cvtColor(black_background, cv2.COLOR_BGR2RGB))  # 转换为RGB以正确显示
plt.axis('off')

# plt.tight_layout()
# plt.show()

plt.savefig(f'./datanew/20240319转换/{ID}.png')
...
# 第一步：读取Excel文件中的数据
data = pd.read_excel('./Fish_Object_Pixel_Counts.xlsx')
...
# 计算特征和目标变量的Z-score
z_scores = np.abs(stats.zscore(data[['Area1', 'Area2', 'Weight (g)']]))
...
# 准备过滤后的特征和目标变量
X_filtered = data_filtered[['Area1', 'Area2']]
y_filtered = data_filtered['Weight (g)']
